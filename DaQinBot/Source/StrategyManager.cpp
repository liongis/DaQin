...
const MetaPairVector StrategyManager::getProtossBuildOrderGoal()
{
    ...
    if (getCarrierCapacity)
    {
		...
		if (UnitUtil::GetCompletedUnitCount(BWAPI::UnitTypes::Protoss_Cybernetics_Core) > 0
			&& !startedStargate && numStargates * 3 < numNexusCompleted){
...
void StrategyManager::handleMacroProduction(BuildOrderQueue & queue)
{
        ...
        !isRushing())
    {
		...
		if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Terran) {
			if (nexuses >= 2 && frame < 7 * 60 * 24 && BWAPI::Broodwar->self()->minerals() < 600) {
				skipThisItem = true;
			}

			if (nexuses >= 3 && frame < 11 * 60 * 24 && BWAPI::Broodwar->self()->minerals() < 600 && CombatCommander::Instance().getNumCombatUnits() < 100) {
				skipThisItem = true;
			}
		}
