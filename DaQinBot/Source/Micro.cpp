...
void Micro::KiteTarget(BWAPI::Unit rangedUnit, BWAPI::Unit target)
{
	...
	if (range > targetRange && rangedUnit->getDistance(target->getPosition()) <= targetRange) {
		BWAPI::Position fleePosition = MapTools::Instance().getExtendedPosition(rangedUnit->getPosition(), target->getPosition(), 1 * 32); (rangedUnit, target);
		//Micro::RightClick(rangedUnit, fleePosition);
		InformationManager::Instance().getLocutusUnit(rangedUnit).fleeFrom(fleePosition);
		return;
	}
