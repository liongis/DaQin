...
void MicroDragoon::assignTargets(const BWAPI::Unitset & targets)
{
	...
	for (const auto rangedUnit : rangedUnits)
	{
		if (order.isCombatOrder())
		{
			...
			BWAPI::Unit closestUnit = rangedUnit->getClosestUnit(BWAPI::Filter::IsEnemy && BWAPI::Filter::CanAttack && BWAPI::Filter::GetType != BWAPI::UnitTypes::Terran_Vulture_Spider_Mine, rangedUnit->getType().sightRange());
